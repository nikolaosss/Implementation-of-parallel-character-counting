...
//we are cheking if the unput is valid. we only want the input to be "add %d worker/s","remove %d worker/s","process"
...
void frontend_init(pid_t dispatcher_pid, int fed[]){
    ...
    signal(SIGUSR1, signal_handler4);//if the dispatcher wants us to send some data, a signal will be triggered
    ...
    while(1){
        ...
        fgets(input, sizeof(input), stdin);// wait for an input from the command
        ...
        if(strstr(input, "add") != NULL){
            ...
            if(valid_input(input, &numDigits)){
                ...
                write(fed[1], output, strlen(output) + 1);//we send him the number of workers he needs to add

                kill(dispatcher_pid, SIGUSR1);//we send a signal to the dispatcher that we have sent him some data, so he goes to read it 
                continue;
            }
            ...
            if(valid_input(input, &numDigits)){
                ...
                snprintf(output, sizeof(output), "%d", j);
                write(fed[1], output, strlen(output) + 1);//we send him the number of workers he needs to add
                kill(dispatcher_pid, SIGUSR2);//we send a signal to the dispatcher that we have sent him some data, so he goes to read it
                continue;
            }
            else{printf("--Invalid input, try again--\n");continue;}
        }

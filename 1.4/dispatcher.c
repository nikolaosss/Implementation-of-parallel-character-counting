...
void signal_handler1(int signum){
    ...
    flag = 1;//flag=1 so we exit from the while we where before this siganl was triggered
...
void calculate(int fd, off_t original_size, int **pipefd, int overall_workers, int cc){
    ...
    if((cc == 1) && (offset == temp_offset + offset_step*5*overall_workers + temp_remainder)){
        ...
        count++;//we are incrementing each time a worker is send the dipatcher an integer
        counter++;//we are incrementing by one each time to send the data to the next worker
    }   
}
...
int dispatcher_init(pid_t f, int fed []){
    ...
    int fd = open("1.txt", O_RDONLY);// we open the txt file and getting the fd

    off_t file_size = lseek(fd, 0, SEEK_END);// we are finding the original size of the file we are about to search
    ...
    signal(SIGUSR1, signal_handler1);//if the command wants to add a number of workers, this siganl is triggered
    signal(SIGUSR2, signal_handler2);//if the command wants to remove a number of workers, this siganl is triggered
    signal(SIGINT, signal_handler3);//if the command wants to know the process until now, this signal is triggered
    ...
    while(1){
        ...
        if(type == 1){//this if is for the case when we want to add workers
            ...
            if(no_change == 0){
                ...
                for(int i = 0;i<number;i++){
                    ...
                    if(p[i+temp] == -1){
                        ...
                        //the parameters we are giving to this funtion ara the fd, the read end of the worker's-dispatcher pipe and the write end of the pipe x
            ...
            while(percentage_read < 100.0){
                ...
                //we also put this here, in case the any signal was triggered when we where in the while loop in the funtion calculate
                if(flag == 1){
                    break;
                }
            ...
            // we are closign the read end of the pipe x, cause the dispatcher will only recieve data from this pipe 
            close(x[1]);
